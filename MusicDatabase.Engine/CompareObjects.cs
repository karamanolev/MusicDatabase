...
//Security restriction in Silverlight prevents getting private properties and fields
        ...
        public bool EmptyStringEqualsNull { get; set; }
        ...
        private bool IsEquivalentToNull(object obj)
        {
            if (this.EmptyStringEqualsNull && obj.Equals(""))
            {
                return true;
            }
            return false;
        ...
        private void Compare(object object1, object object2, string breadCrumb)
        {
            ...
            if (object1 == null)
            {
                if (this.IsEquivalentToNull(object2))
                {
                    return;
                }
            ...
            if (object2 == null)
            {
                if (this.IsEquivalentToNull(object1))
                {
                    return;
                }
            ...
            string breadCrumb)
        {
            ...
            foreach (FieldInfo item in currentFields)
            {
                ...
                //If we should ignore it, skip it
                if (ElementsToIgnore.Contains(item.Name) || IgnoredByAttribute(item.FieldType) || IgnoredByAttribute(item))
                    continue;
            ...
            string breadCrumb)
        {
            ...
            foreach (PropertyInfo info in currentProperties)
            {
                ...
                //If we should ignore it, skip it
                if (ElementsToIgnore.Contains(info.Name) || IgnoredByAttribute(info.PropertyType) || IgnoredByAttribute(info))
                    continue;
